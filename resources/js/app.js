...
function renderNotes(notes) {
    state.visibleCount = notes.length;
    elements.notes.innerHTML = notes.map((note) => `
        <article class="note-card">
            <div>
                <div class="note-head">
                    <h2>${escapeHtml(note.title)}</h2>
                    ${note.score ? `<span class="score">${Math.round(note.score * 100)}% match</span>` : ''}
                </div>
...
function updatePager() {
    elements.pageLabel.textContent = state.semanticMode
        ? `${state.visibleCount} semantic result${state.visibleCount === 1 ? '' : 's'}`
...
async function submitNote(event) {
    ...
    try {
        setFormLoading(true, id ? 'Updating...' : 'Saving...');
        await request(url, {
            method,
            body: JSON.stringify(formPayload()),
        });
        resetForm();
        elements.formSuccess.textContent = id ? 'Note updated successfully.' : 'Note saved successfully.';
...
function setFormLoading(isLoading, label = 'Save') {
    elements.form?.classList.toggle('is-loading', isLoading);
    if (elements.saveNote) {
        elements.saveNote.disabled = isLoading;
        elements.saveNote.querySelector('.button-label').textContent = label;
    }

    elements.form?.querySelectorAll('input, textarea, button').forEach((control) => {
        if (control.id !== 'reset-form') {
